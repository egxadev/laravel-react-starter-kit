...
export function DataTable<TData>({
    ...
    const [search, setSearch] = React.useState((filters.search as string) ?? '');
    const [sorting, setSorting] = React.useState<SortingState>([
        { id: (filters.sort_by as string) ?? 'created_at', desc: filters.sort_dir === 'desc' },
    ]);
    ...
    const handleServerOperation = React.useCallback(
        (params: { page?: number; per_page?: number; sort_by?: string; sort_dir?: string; search?: string; [key: string]: unknown }) => {
            router.get(
                toUrl(routeUrl),
                {
                    ...filters,
    ...
    React.useEffect(() => {
        ...
        const timer = setTimeout(() => handleServerOperation({ search, page: 1 }), 500);
    ...
    React.useEffect(() => {
        if (isInitialRender) return;
        if (sorting.length > 0) {
            handleServerOperation({ sort_by: sorting[0].id, sort_dir: sorting[0].desc ? 'desc' : 'asc', page: 1 });
        }
    }, [sorting]);
    ...
    const table = useReactTable({
        ...
        state: {
            sorting,
            pagination: { pageIndex: meta.current_page - 1, pageSize: meta.per_page },
            columnVisibility,
            rowSelection,
        },
    });
        ...
        <div className="w-full px-2 sm:px-4">
            <div className="flex flex-col gap-2 py-4 sm:flex-row sm:items-center sm:gap-4">
                ...
                <div className="flex w-full flex-row gap-1 sm:w-auto">
                    ...
                    {createHref && (
                        <a href={toUrl(createHref)} className="w-full sm:w-auto">
                            <Button className="w-full sm:w-auto">{createLabel}</Button>
                        </a>
                    )}
                </div>
            </div>
